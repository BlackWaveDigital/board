...
| Kevin | Monster | danbox | danbox | Assigned | 2026-03-06 | CHS #13 + Ellianos E7 |
| Nick | Monster | juhnk | juhnk | Assigned | 2026-03-06 | CHS #8 + Ellianos E5 |
...
| Doug | Monster | hurb-8 | hurb-8 | Assigned | 2026-03-06 | CHS #2: Approval chain state machine |
| Atihar | Monster | hurb-9 | hurb-9 | Assigned | 2026-03-06 | CHS #11: Permissions service upgrade |
| Ali | Monster | hurb-10 | hurb-10 | Assigned | 2026-03-06 | CHS #3: Level-specific approval queues |
...
| E5 | **Content Studio access for location users** — Enable Content Studio in sidebar/routes for location-level users. They can browse, view, download brand assets. Creation permissions TBD (may allow organic social content creation). | **juhnk** | Nick | P1 | As location user: verify Content Studio appears in sidebar. Can browse assets, view previews, download. Verify no destructive actions available unless explicitly granted. |
| E6 | **Meta account connection for Ellianos** — Connect Ellianos Meta Business account for analytics sync. Submit Meta app review AND add as test user as fallback. | **danbox** | Kevin | P1 | After connection: verify Meta analytics data appears on Ellianos dashboard. Check daily sync runs. Verify location-level attribution if available. |
| E7 | **Ellianos company + locations + users setup** — Create Ellianos company, add locations (coffee shops), create brand admin + location user accounts, assign roles. | **danbox** | Kevin | P1 | Log in as each role level. Verify correct dashboard shell loads. Verify location users only see their location's data. Brand admin sees all locations. |
